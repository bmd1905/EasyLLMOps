    ...
    def load_prompts(self) -> Dict[str, BaseModel]:
        ...
        for prompt_file in prompt_files:
            if prompt_file.endswith('.py') and prompt_file != '__init__.py':
                ...
                prompt_module = path_config.PROMPT_MODULE.format(prompt_name=prompt_name)
                try:
...
class PromptLoader:
    ...
    def __init__(self, prompt_dir: str = path_config.PROMPT_BASE_DIR, strategy: PromptLoaderStrategy = None):
        """
        Initialize the `PromptLoader` with a directory and a strategy for loading prompts.

        Args:
            prompt_dir (str): Directory where prompt files are located. Defaults to `path_config.PROMPT_BASE_DIR`.
